...
const semanticQueryRules: Array<{ pattern: RegExp; variants: string[] }> = [
  ...
  { pattern: /(龙卷风|龍捲風|tornado)/i, variants: ['tornado', 'tornado safety', 'severe weather'] },
  { pattern: /(飓风|颶風|台风|颱風|hurricane|typhoon)/i, variants: ['hurricane', 'storm surge', 'severe weather'] },
  { pattern: /(海啸|海嘯|tsunami)/i, variants: ['tsunami', 'coastal evacuation'] },
  { pattern: /(火山|火山灰|volcano|volcanic ash)/i, variants: ['volcano', 'volcanic ash', 'evacuation'] },
  { pattern: /(雪崩|avalanche)/i, variants: ['avalanche', 'winter weather'] },
  { pattern: /(山火|野火|wildfire|wild fire|wildland fire)/i, variants: ['wildfire', 'wildfire smoke', 'evacuation'] },
  {
    pattern: /(化学品|化學品|化学泄漏|化學洩漏|化学暴露|化學暴露|chemical spill|chemical exposure|household chemical|hazmat)/i,
    variants: ['household chemical emergencies', 'chemical exposure', 'hazmat', 'ventilation'],
  },
  {
    pattern: /(灾后清理|災後清理|洪水后|洪水後|发霉|發霉|霉菌|mold cleanup|disaster cleanup|flood cleanup)/i,
    variants: ['disaster cleanup', 'mold cleanup', 'flood cleanup', 'respiratory protection'],
  },
